...
class DATA_LOADER(object):
    ...
    def read_matdataset(self, opt):
        ...
        if opt.dataset == 'AWA1':
            self.name = matcontent['allclasses_names']
        ...
        self.tr_cls_centroid = np.zeros([self.ntrain_class, self.train_feature.shape[1]]).astype(np.float32)
        for i in range(self.ntrain_class):
            self.tr_cls_centroid[i] = torch.mean(self.train_feature[self.train_mapped_label == i], axis=0)
